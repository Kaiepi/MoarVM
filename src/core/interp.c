           ...
           OP(bootintarray):
            ...
            OP(bind_sk):
                MVM_io_bind(tc,
                    GET_REG(cur_op, 0).o, GET_REG(cur_op, 2).o,
                    GET_REG(cur_op, 4).i64, GET_REG(cur_op, 6).i64, GET_REG(cur_op, 8).i64,
                    (MVMint32)GET_REG(cur_op, 10).i64);
                cur_op += 12;
                goto NEXT;
            OP(accept_sk):
            ...
            OP(asynclisten):
                GET_REG(cur_op, 0).o = MVM_io_socket_listen_async(tc,
                    GET_REG(cur_op, 2).o, GET_REG(cur_op, 4).o,
                    GET_REG(cur_op, 6).o, GET_REG(cur_op, 8).i64, (MVMint32)GET_REG(cur_op, 10).i64,
                    GET_REG(cur_op, 12).o);
                cur_op += 14;
                goto NEXT;
            OP(asyncwritebytes):
            ...
            OP(asyncudp):
                GET_REG(cur_op, 0).o = MVM_io_socket_udp_async(tc,
                    GET_REG(cur_op, 2).o, GET_REG(cur_op, 4).o,
                    GET_REG(cur_op, 6).o, GET_REG(cur_op, 8).i64, GET_REG(cur_op, 10).i64,
                    GET_REG(cur_op, 12).o);
                cur_op += 14;
                goto NEXT;
            OP(asyncwritebytesto):
                GET_REG(cur_op, 0).o = MVM_io_write_bytes_to_async(tc,
            ...
            OP(addrfromstr_ip4):
                GET_REG(cur_op, 0).o = MVM_address_from_ipv4_literal(tc,
                    GET_REG(cur_op, 2).s, (MVMuint16)GET_REG(cur_op, 4).i64);
                cur_op += 6;
                goto NEXT;
            OP(addrfromstr_ip6):
                GET_REG(cur_op, 0).o = MVM_address_from_ipv6_literal(tc,
                    GET_REG(cur_op, 2).s, (MVMuint16)GET_REG(cur_op, 4).i64);
                cur_op += 6;
                goto NEXT;
            OP(addrfromstr_un):
            ...
            OP(addrtostr): {
                ...
                if (REPR(address)->ID == MVM_REPR_ID_MVMAddress && IS_CONCRETE(address))
                    ...
                    MVM_exception_throw_adhoc(tc,
                        "addrtostr requires a concrete object with REPR MVMAddress, got %s (%s)",
            ...
            OP(addrfrombuf_ip4): {
                ...
                if (REPR(address_buf)->ID == MVM_REPR_ID_VMArray && IS_CONCRETE(address_buf))
                    GET_REG(cur_op, 0).o = MVM_address_from_ipv4_address(tc,
                        (MVMArray *)address_buf, (MVMuint16)GET_REG(cur_op, 4).i64);
                else
                    MVM_exception_throw_adhoc(tc,
                        "addrfrombuf_ip4 requires a concrete object with REPR VMArray, got %s (%s)",
            ...
            OP(addrfrombuf_ip6): {
                ...
                if (REPR(address_buf)->ID == MVM_REPR_ID_VMArray && IS_CONCRETE(address_buf))
                    GET_REG(cur_op, 0).o = MVM_address_from_ipv6_address(tc,
                        (MVMArray *)address_buf, (MVMuint16)GET_REG(cur_op, 4).i64, (MVMuint32)GET_REG(cur_op, 6).i64);
                else
                    MVM_exception_throw_adhoc(tc,
                        "addrfrombuf_ip6 requires a concrete object with REPR VMArray, got %s (%s)",
                        REPR(address_buf)->name, MVM_6model_get_debug_name(tc, address_buf));
                cur_op += 8;
                goto NEXT;
            }
            ...
            OP(addrtobuf): {
                ...
                if (REPR(address)->ID != MVM_REPR_ID_MVMAddress || !IS_CONCRETE(address))
                    MVM_exception_throw_adhoc(tc,
                        "addrtobuf requires a concrete object with REPR MVMAddress, got %s (%s)",
                        REPR(address)->name, MVM_6model_get_debug_name(tc, buf_type));
                else if (REPR(buf_type)->ID != MVM_REPR_ID_VMArray)
                    MVM_exception_throw_adhoc(tc,
                        "addrtobuf requires a type object with REPR VMArray, got %s (%s)",
                        REPR(buf_type)->name, MVM_6model_get_debug_name(tc, buf_type));
                else
                    GET_REG(cur_op, 0).o = MVM_address_to_buf(tc, (MVMAddress *)address, (MVMArray *)buf_type);
                cur_op += 6;
                goto NEXT;
            }
            OP(dnsresolve): {
