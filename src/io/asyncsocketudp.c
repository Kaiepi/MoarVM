...
static void on_read(uv_udp_t *handle, ssize_t nread, const uv_buf_t *buf, const struct sockaddr *addr, unsigned flags) {
    ...
    if (nread >= 0) { /* Read OK! */
        MVMROOT2(tc, t, arr, {
            ...
            /* Push a freshly produced buffer: */
            ...
            /* Push the peer address: */
            address = (MVMAddress *)MVM_repr_alloc_init(tc, tc->instance->boot_types.BOOTAddress);
            memcpy(&address->body.storage, addr, MVM_address_get_storage_length(tc, addr));
            MVM_repr_push_o(tc, arr, (MVMObject *)address);
        });
    }
    else if (nread == UV_EOF) { /* Reading done. */
        /* Push a null error message: */
        MVM_repr_push_o(tc, arr, tc->instance->boot_types.BOOTStr);
        /* Push a null buffer: */
        MVM_repr_push_o(tc, arr, ri->buf_type);
        /* Push the sequence number: */
        MVMROOT2(tc, t, arr, {
            MVM_repr_push_o(tc, arr, MVM_repr_box_int(tc,
                tc->instance->boot_types.BOOTInt, ri->seq_number));
        });
        /* Push a null peer address: */
    ...
    else { /* An error occurred while reading. */
...
static MVMAsyncTask * read_bytes(MVMThreadContext *tc, MVMOSHandle *h, MVMObject *queue,
    ...
    MVM_ASSIGN_REF(tc, &(task->common.header), ri->buf_type, STABLE(buf_type)->WHAT);
...
static void on_write(uv_udp_send_t *req, int status) {
    ...
    if (status == 0) { /* Write OK! */
        /* Push a null error message: */
        MVM_repr_push_o(tc, arr, tc->instance->boot_types.BOOTStr);
        /* Push the write length: */
        MVMROOT2(tc, arr, t, {
            MVM_repr_push_o(tc, arr, MVM_repr_box_int(tc,
                tc->instance->boot_types.BOOTInt, wi->buf.len));
        });
    }
    else { /* An error occurred while writing. */
        /* Push the error message: */
        MVMROOT2(tc, arr, t, {
            MVMString *msg_str = MVM_string_ascii_decode_nt(tc,
...
static void write_setup(MVMThreadContext *tc, uv_loop_t *loop, MVMObject *async_task, void *data) {
    ...
    if ((r = uv_udp_send(wi->req, handle_data->handle, &(wi->buf), 1, &wi->address->body.storage.any, on_write)) < 0) {
        /* Error; need to notify. */
        MVMROOT(tc, async_task, {
            MVMObject *arr = MVM_repr_alloc_init(tc, tc->instance->boot_types.BOOTArray);
            MVM_repr_push_o(tc, arr, ((MVMAsyncTask *)async_task)->body.schedulee);
...
static MVMAsyncTask * write_bytes_to(MVMThreadContext *tc,
        MVMOSHandle *h, MVMObject *queue, MVMObject *schedulee,
        MVMAddress *address, MVMObject *buffer,
    ...
    if (!IS_CONCRETE(address) || REPR(address)->ID != MVM_REPR_ID_MVMAddress)
        MVM_exception_throw_adhoc(tc,
            "asyncwritebytesto address must be an object with the MVMAddress REPR");
...
static void setup_setup(MVMThreadContext *tc, uv_loop_t *loop, MVMObject *async_task, void *data) {
    ...
    int r;
    if ((r = uv_udp_init_ex(loop, udp_handle, (unsigned int)ssi->family->native)) >= 0) {
        if (ssi->address)
            r = uv_udp_bind(udp_handle, &ssi->address->body.storage.any, 0);
        if (r >= 0 && (ssi->flags & 1))
...
/* Marks objects for an async UDP socket task. */
...
/* Frees info for an async UDP socket task. */
static void setup_gc_free(MVMThreadContext *tc, MVMObject *t, void *data) {
    if (data)
        MVM_free(data);
}

/* Operations table for async UDP socket task. */
...
MVMObject * MVM_io_socket_udp_async(MVMThreadContext *tc,
        ...
        MVMObject *address, MVMint64 family_value, MVMint64 flags,
        MVMObject *async_type) {
    MVMint64         has_address;
    MVMAsyncTask    *task;
    SocketSetupInfo *ssi;

    /* Validate REPRs. */
    has_address = !MVM_is_null(tc, address);
    if (REPR(queue)->ID != MVM_REPR_ID_ConcBlockingQueue)
        MVM_exception_throw_adhoc(tc,
            "asyncudp target queue must have ConcBlockingQueue REPR");
    if (has_address && (REPR(address)->ID != MVM_REPR_ID_MVMAddress || !IS_CONCRETE(address)))
        MVM_exception_throw_adhoc(tc,
            "asyncudp address must be an object with the MVMAddress REPR");
    if (REPR(async_type)->ID != MVM_REPR_ID_MVMAsyncTask)
        MVM_exception_throw_adhoc(tc,
    ...
    ssi->family     = MVM_io_socket_runtime_family(tc, family_value);
    if (has_address)
        MVM_ASSIGN_REF(tc, &(task->common.header), ssi->address, address);
    ssi->flags      = flags;
    task->body.data = ssi;
