...
static void write_setup(MVMThreadContext *tc, uv_loop_t *loop, MVMObject *async_task, void *data) {
    ...
    if ((r = uv_udp_send(wi->req, handle_data->handle, &(wi->buf), 1, (struct sockaddr *)&wi->address->body.storage, on_write)) < 0) {
        /* Error; need to notify. */
        MVMROOT(tc, async_task, {
            MVMObject    *arr = MVM_repr_alloc_init(tc, tc->instance->boot_types.BOOTArray);
            MVM_repr_push_o(tc, arr, ((MVMAsyncTask *)async_task)->body.schedulee);
...
static MVMAsyncTask * write_bytes_to(MVMThreadContext *tc,
    ...
    MVMROOT6(tc, h, queue, schedulee, buffer, async_type, address, {
        task = (MVMAsyncTask *)MVM_repr_alloc_init(tc, async_type);
        MVM_ASSIGN_REF(tc, &(task->common.header), task->body.queue, queue);
        MVM_ASSIGN_REF(tc, &(task->common.header), task->body.schedulee, schedulee);
...
static MVMObject * socket_getsockname(MVMThreadContext *tc, MVMOSHandle *h) {
    ...
    if ((error = uv_udp_getsockname(data->handle, (struct sockaddr *)&address->body.storage, &address_len)))
...
static MVMObject * socket_getpeername(MVMThreadContext *tc, MVMOSHandle *h) {
    ...
    data    = (MVMIOAsyncUDPSocketData *)h->body.data;
    address = (MVMAddress *)MVM_repr_alloc_init(tc, tc->instance->boot_types.BOOTAddress);
    if ((error = uv_udp_getpeername(data->handle, (struct sockaddr *)&address->body.storage, &address_len)))
...
static void setup_setup(MVMThreadContext *tc, uv_loop_t *loop, MVMObject *async_task, void *data) {
    ...
    if ((r = uv_udp_init(loop, udp_handle)) >= 0) {
        if (ssi->address)
            r = uv_udp_bind(udp_handle, (struct sockaddr *)&ssi->address->body.storage, 0);
        if (r >= 0 && (ssi->flags & 1))
...
MVMObject * MVM_io_socket_udp_async(MVMThreadContext *tc,
    ...
    if (REPR(queue)->ID != MVM_REPR_ID_ConcBlockingQueue)
        MVM_exception_throw_adhoc(tc,
            "asyncudp target queue must have the ConcBlockingQueue REPR");
    if (REPR(async_type)->ID != MVM_REPR_ID_MVMAsyncTask)
        MVM_exception_throw_adhoc(tc,
            "asyncudp result type must have the AsyncTask REPR");
    if (!passive && REPR(address)->ID != MVM_REPR_ID_MVMAddress)
        MVM_exception_throw_adhoc(tc,
    ...
    MVMROOT4(tc, queue, schedulee, async_type, address, {
        ...
        MVM_ASSIGN_REF(tc, &(task->common.header), ssi->address, passive ? NULL : address);
        ssi->flags      = flags;
        task->body.data = ssi;
