...
static void on_write(uv_write_t *req, int status) {
    ...
    else { /* Error occurred during the write. */
        /* Push the error message: */
        MVMROOT2(tc, arr, t, {
            MVMString *msg_str = MVM_string_ascii_decode_nt(tc,
...
static void write_setup(MVMThreadContext *tc, uv_loop_t *loop, MVMObject *async_task, void *data) {
    ...
    if ((r = uv_write(wi->req, handle_data->handle, &(wi->buf), 1, on_write))) {
        /* Error; need to notify. */
        MVMROOT(tc, async_task, {
            MVMObject *arr = MVM_repr_alloc_init(tc, tc->instance->boot_types.BOOTArray);
            MVM_repr_push_o(tc, arr, ((MVMAsyncTask *)async_task)->body.schedulee);
...
static MVMObject * socket_getsockname(MVMThreadContext *tc, MVMOSHandle *h) {
    const MVMIOAsyncSocketData *data;
    struct sockaddr_storage     address_storage;
    socklen_t                   address_len;
    int                         error;

    data        = (const MVMIOAsyncSocketData *)h->body.data;
    address_len = sizeof(address_storage);
    if ((error = uv_tcp_getsockname((uv_tcp_t *)data->handle, (struct sockaddr *)&address_storage, &address_len)))
        MVM_exception_throw_adhoc(tc,
...
static MVMObject * socket_getpeername(MVMThreadContext *tc, MVMOSHandle *h) {
    ...
    data        = (const MVMIOAsyncSocketData *)h->body.data;
    address_len = sizeof(address_storage);
    if ((error = uv_tcp_getpeername((uv_tcp_t *)data->handle, (struct sockaddr *)&address_storage, &address_len)))
        MVM_exception_throw_adhoc(tc,
...
static void on_connect(uv_connect_t* req, int status) {
    ...
    else { /* An error occurred while connecting. */
        /* Push the error message: */
        MVMROOT2(tc, arr, t, {
            MVMString *msg_str = MVM_string_ascii_decode_nt(tc,
                tc->instance->VMString, uv_strerror(status));
...
static void connect_setup(MVMThreadContext *tc, uv_loop_t *loop, MVMObject *async_task, void *data) {
    ...
    if ((r = uv_tcp_init_ex(loop, ci->socket, (unsigned int)ci->family->native)) ||
        (r = uv_tcp_connect(ci->connect, ci->socket, &ci->address->body.storage.any, on_connect))) {
...
static void on_connection(uv_stream_t *server, int status) {
    ...
    MVM_repr_push_o(tc, arr, t->body.schedulee);
    if ((r = uv_accept(server, (uv_stream_t *)client)) == 0) { /* Accept OK! */
        /* Push a null error message: */
        MVM_repr_push_o(tc, arr, tc->instance->boot_types.BOOTStr);
        /* Push a null binding: */
        MVM_repr_push_o(tc, arr, tc->instance->boot_types.BOOTIO);
    ...
    else { /* An error occurred while listening. */
        /* Push the error message: */
        MVMROOT2(tc, arr, t, {
            MVMString *msg_str = MVM_string_ascii_decode_nt(tc,
                tc->instance->VMString, uv_strerror(r));
...
static void listen_setup(MVMThreadContext *tc, uv_loop_t *loop, MVMObject *async_task, void *data) {
    ...
    if (((r = uv_tcp_init_ex(loop, li->socket, (unsigned int)li->family->native)) == 0) &&
        ...
        ((r = uv_listen((uv_stream_t *)li->socket, li->backlog, on_connection)) == 0)) {
        /* Bind and listen OK! */
        MVMROOT(tc, async_task, {
            MVMObject *arr = MVM_repr_alloc_init(tc, tc->instance->boot_types.BOOTArray);
            MVM_repr_push_o(tc, arr, ((MVMAsyncTask *)async_task)->body.schedulee);
        ...
        /* An error occurred at some point while binding and listening. */
        MVMROOT(tc, async_task, {
            MVMObject    *arr = MVM_repr_alloc_init(tc, tc->instance->boot_types.BOOTArray);
            MVM_repr_push_o(tc, arr, ((MVMAsyncTask *)async_task)->body.schedulee);
            /* Push the error message: */
